<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto p-4 sm:p-6 lg:p-8">
        <form id="task-form" method="POST" action="{{ route('tasks.store', $project) }}">
            @csrf
            <input
                name="message"
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            @if ($task->user->id === auth()->user()->id )
                                <x-dropdown>
                                    ...
                                    <x-slot name="content">
                                        <x-dropdown-link :href="route('tasks.edit', ['task' =>$task, 'project' =>$project->id])">
                                            {{ __('Edit') }}

                                            <form method="POST" action="{{ route('tasks.destroy',  ['task' =>$task, 'project' =>$project->id]) }}">
                                                @csrf
                                                @method('delete')
                                                <x-dropdown-link :href="route('tasks.destroy',  ['task' =>$task, 'project' =>$project->id])" onclick="event.preventDefault(); this.closest('form').submit();">
                                                    {{ __('Delete') }}
                                                </x-dropdown-link>
                                            </form>

                                            <form method="POST" action="{{ route('tasks.markAsCompleted',  ['task' =>$task, 'project' =>$project->id]) }}">
                                                 @csrf
                                                <x-dropdown-link :href="route('tasks.markAsCompleted',  ['task' =>$task, 'project' =>$project->id])" onclick="event.preventDefault(); this.closest('form').submit();">
                                                    @if ($task->completed)
