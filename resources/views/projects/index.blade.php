<x-app-layout>
    <div class="max-w-2xl mx-auto p-4 sm:p-6 lg:p-8">
        <form id="task-form" method="POST" action="{{ route('projects.store') }}">
            @csrf
            <input
                name="message"
            ...
            <x-primary-button  id="submit-button" class="mt-4">{{ __('created project') }}</x-primary-button>
        </form>
        @if(session('success'))
    ...
    @endif
        ...
        <div class="mt-6 bg-white shadow-sm rounded-lg divide-y">
            ...
            <a href="{{route('projects.show', $project)}}">{{ $project->message }}</a>
                    ...
                    <div class="flex-1">
                        <div class="flex justify-between items-center">
                            <div>
                                ...
                                @endunless
                                <a href="{{ route('projects.show', $project) }}" class="text-blue-500 hover:underline">                                </a>
                                ...
                                <x-dropdown>
                                    ...
                                    <x-slot name="content">
                                        <x-dropdown-link :href="route('projects.edit', $project)">
                                            ...
                                            <form method="POST" action="{{ route('projects.markAsCompleted',$project) }}">
                                                 @csrf
                                                <x-dropdown-link :href="route('projects.markAsCompleted', $project)" onclick="event.preventDefault(); this.closest('form').submit();">
        ...
        <br>
        <div class="mt-6 bg-white shadow-sm rounded-lg divide-y">
            <h3 class="text-2xl font-bold mb-4 text-center text-blue-600">{{ __('Assigned Project') }}</h3>
                ...
                <div class="p-6 flex space-x-2">
                    ...
                    <div class="flex-1">
                        <div class="flex justify-between items-center">
                            <div>
                                <span class="text-gray-800">{{ $project->user->name }}</span>
                                <a href="{{ route('projects.assignedTasks', $project) }}" class="text-blue-500 hover:underline">{{ __('Tasks for project') }}</a>
                                <small class="ml-2 text-sm text-gray-600">{{ $project->created_at->format('j M Y, g:i a') }}</small>
                                @unless ($project->created_at->eq($project->updated_at))
                                    <small class="text-sm text-gray-600"> &middot; {{ __('edited') }}</small>
                                @endunless
